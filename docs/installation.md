...
??? info "macOS instructions (click to expand)"
    ...
    [Download](https://github.com/ZcashFoundation/zec-qt-wallet/releases) and double-click on the .dmg file to open it, and drag zec-qt-wallet on to the Applications link to install. You will need to give authorisation for the program to run.
...
??? info "Linux instructions (click to expand)"
    ...
    ``` bash
    sudo dpkg -i linux-deb-zec-qt-wallet-vx.x.x.deb
    sudo apt install -f
    ```
    ...
    ``` bash
    tar -xvf zec-qt-wallet-vx.x.x.tar.gz
    ./zec-qt-wallet-vx.x.x/zec-qt-wallet
    ``` 
